...
<div class="container-fluid px-0">
    ...
    <?php if ($lowStockCount > 0): ?>
        <div class="card-custom mb-4">
            <div class="card-header-custom" style="background: #dc3545; color: white;">
                <i class="fas fa-exclamation-triangle me-2"></i> Low Stock Alert
                <span class="badge bg-light text-dark ms-2"><?= $lowStockCount ?> items</span>
            </div>
            ...
            <td><?= htmlspecialchars($book['title']) ?>
        </div>
        <td><span class="badge bg-danger"><?= $book['stock'] ?></span>
</div>
<td><?= $book['min_stock'] ?></div>
<td>
    <?php if ($book['stock'] <= 0): ?>
        <span class="badge bg-danger">Out of Stock</span>
    <?php elseif ($book['stock'] <= $book['min_stock'] / 2): ?>
        <span class="badge bg-warning text-dark">Very Low</span>
    <?php else: ?>
        <span class="badge bg-warning text-dark">Low</span>
    <?php endif; ?>
    </div>
<td>
    <a href="../Admin/manageBook.php?edit=<?= $book['id'] ?>&restock=<?= $book['id'] ?>" class="btn-sm-primary" style="padding: 0.3rem 0.8rem;">
                ...
                <table class="table table-bordered table-custom mb-0">
                    ...
                    <tbody>
                        <?php if (empty($recentOrders)): ?>
                            <tr>
                                <td colspan="6" class="text-center">No orders yet
            </div>
</td>
<?php else: foreach ($recentOrders as $order): ?>
    <tr>
        ...
        <td>RM <?= number_format($order['total_amount'], 2) ?></div>
        <td>
        ...
        <td>
            <span class="badge-status bg-<?= $order['payment_status'] == 'paid' ? 'success' : 'secondary' ?>">
...
<div class="p-2 text-end border-top">
    <a href="staffManage.php" class="btn-sm-primary" style="padding: 0.4rem 1rem;">View All Orders</a>
</div>
</div>
</div>
<div class="col-md-4">
